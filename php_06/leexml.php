...
<html>
	...
	<body style="background-color: #FFFFCC; color: #800000">
		...
		<?php
			// Obtiene la ruta completa del archivo XML usando DOCUMENT_ROOT
			$ruta_xml = $_SERVER["DOCUMENT_ROOT"]."/WEB3/php_06/xml/generales_5.xml";

			// Carga el archivo XML con la ruta absoluta
			if (file_exists($ruta_xml)) {
				$inventario = simplexml_load_file($ruta_xml);
				lectu_recursiva($inventario);
			} else {
				echo "Error: No se encontró el archivo XML en la ruta especificada.";
			}
		?>
	</body>
</html>